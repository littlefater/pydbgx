...
class Function:
    ...
    def __format_param(self, param):
        ...
        if -1 == param.find(' '):
            raise Exception('Function parameters can not be formatted: Error format.')
        
        item = param.split(' ')
        if len(item) < 3:
            raise Exception('Function parameters can not be formatted: Error number.')
...
def generate_idl_file(filename, interfaces):
    ...
    out += '// https://support.microsoft.com/en-us/kb/241849\r\n'
    out += '#if (__midl >= 501)\r\n'
    out += 'midl_pragma warning( disable: 2362 )\r\n'
    out += '#endif\r\n\r\n'
